...
# react-svg
- For SVG React components, follow the Lock.tsx pattern: DEFAULT_COLORS with `as const`, derived type via mapped type, Props with className/colors/animate, useId() for unique IDs, and useReducedMotion() for accessibility. Confidence: 0.80
